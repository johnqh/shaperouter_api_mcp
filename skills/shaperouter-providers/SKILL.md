---
name: shaperouter-providers
description: Use when managing ShapeRouter LLM providers, projects, or endpoints with an entity API key - creating or rotating provider keys, creating/updating/deleting endpoints, wiring an endpoint to a provider, or managing the workspace's own API keys. Trigger on /shaperouter-providers, "add an OpenAI key to my workspace", "rotate the Anthropic key", "create an entity API key", "set up CI access to shaperouter", "point this endpoint at a different model", "list my providers", or "why is my endpoint 401/403".
---

# ShapeRouter Providers & Entity Keys

Manage a workspace's LLM provider keys, projects, and endpoints through the
**shaperouter-api** MCP server, authenticating with an **entity API key**
(`shrouteent_...`).

For designing and debugging an individual endpoint's schema and prompt, use the
`shaperouter-endpoint` skill instead — this one is about workspace credentials
...
Set it before starting: `SHAPEROUTER_ENTITY_API_KEY=shrouteent_...`, plus
`SHAPEROUTER_ENTITY_SLUG` so tools can omit `entitySlug`. Or hand it over
mid-session with `set_credentials({ entityApiKey: "shrouteent_...", persist: true })`.

Four credentials exist. Do not mix them up:

| Credential | Looks like | Authenticates as | Used for |
|---|---|---|---|
| Entity API key | `shrouteent_...` | the entity | this skill — providers, projects, endpoints |
| Personal API key | `shroute_...` | a user | user routes, creating entity keys |
...
- **Never echo a secret.** Provider keys are write-only; the API returns
  `has_api_key: boolean`. Do not print a `shrouteent_`/`sk_live_` value into chat,
