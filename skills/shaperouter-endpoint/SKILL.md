---
name: shaperouter-endpoint
description: Use when building, invoking, debugging, or auditing a ShapeRouter structured-output LLM endpoint. Trigger on /shaperouter-endpoint, "create an AI endpoint", "turn this prompt into an API", "I need structured JSON from an LLM", "extract fields from these documents", "classify this with an LLM endpoint", "call my shaperouter endpoint", "test my endpoint", "my endpoint returns the wrong shape", "why is my endpoint returning 401/404/429", "what are my endpoints costing", or "audit my shaperouter setup".
---

# ShapeRouter Endpoint

Build and operate ShapeRouter endpoints. A ShapeRouter endpoint is one LLM
...
caller  →  POST /api/v1/ai/:orgPath/:projectName/:endpointName   (project API key)
              ↓
        ShapeRouter API   builds the prompt from instructions + context + output schema,
...
**shaperouter-api** — 65 tools over the ShapeRouter REST API.

| Group | Tools |
|---|---|
| Docs | `describe_shaperouter_api` (sections: overview, routes, data-model, examples, errors, providers) |
...
`claude plugin install shaperouter@shaperouter`) or add the server to their
settings, and stop.
...
(redacted), and what the local config file `~/.shaperouter/config.json` holds.

The deployed app is **shaperouter.com** and its API is **api.shaperouter.com** — that
...
- a **personal API key** (`shroute_...`) — acts as *you*, and can reach everything
- an **entity API key** (`shrouteent_...`) — acts as the *workspace*, so it keeps
...
keys themselves, see the `shaperouter-providers` skill.

`get_configuration` reports `adminToolsReady` and whether
`~/.shaperouter/config.json` already holds a key. **If it does, you are done — do
not ask the user for anything.**

If `adminToolsReady` is false, or a tool answers "No ShapeRouter credential
configured", stop and prompt with the full instructions:

> "I need a ShapeRouter API key to work with your account. Here's how to create one:
>
> 1. Go to **[https://shaperouter.com](https://shaperouter.com)** and sign in
...
> 5. Copy the key it shows you. It starts with `shroute_`
>
> Then paste it here and I'll save it for future sessions:
> `My shaperouter key is shroute_...`
>
...
> shaperouter.com and tell me the API URL too.)"
...
`persist: true` writes it to `~/.shaperouter/config.json` with mode 0600, so later
...
Read `describe_shaperouter_api({ section: "errors" })` and match the symptom.
...
| Error | Response |
|---|---|
| `No ShapeRouter credential configured` | Walk the user through creating a key at shaperouter.com (prerequisite 2), then `set_credentials({ apiKey, persist: true })` |
...
- `references/authentication.md` — the three credential types, how to create a key on shaperouter.com, and the local config file
...
- `describe_shaperouter_api({ section })` — the full API documentation bundled with the MCP server
