...
ShapeRouter has three credentials. Using the wrong one is the most common cause of
a `401` or `403`, so it is worth knowing which is which.

| Credential | Prefix | Proves | Sent as | Expires |
|---|---|---|---|---|
| Personal API key | `shroute_` | *You* are the caller | `X-API-Key: shroute_...` | never (until revoked) |
...
```bash
curl https://api.shaperouter.com/api/v1/users/me -H "X-API-Key: shroute_..."
curl https://api.shaperouter.com/api/v1/entities -H "X-API-Key: shroute_..."
```
...
2. Environment variables — `SHAPEROUTER_API_KEY`, `SHAPEROUTER_AUTH_TOKEN`,
   `SHAPEROUTER_PROJECT_API_KEY`, `SHAPEROUTER_API_URL`, `SHAPEROUTER_ENTITY_SLUG`,
   `SHAPEROUTER_ORG_PATH`
3. The local config file

### The local config file

```
~/.shaperouter/config.json          (override with SHAPEROUTER_CONFIG_PATH)
```

```json
{
  "apiUrl": "https://api.shaperouter.com",
  "apiKey": "shroute_...",
...
2. `clear_stored_credentials` — remove it from `~/.shaperouter/config.json`.
