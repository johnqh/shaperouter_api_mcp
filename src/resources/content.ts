 ...
 *
 * These strings are the human/agent-readable description of the ShapeRouter API:
...
export const OVERVIEW_MD = `# ShapeRouter API — Overview

ShapeRouter turns an LLM prompt into a **versioned REST endpoint that returns
...
- API: Hono on Bun, PostgreSQL (\`shaperouter\` schema) via Drizzle ORM
...
└── Personal API keys   shroute_... credentials for CLIs, MCP servers, scripts
...
| everything else | Personal API key \`shroute_...\` | \`X-API-Key: shroute_...\` |
...
one published AI endpoint. A *personal* key (\`shroute_...\`) authenticates its owner
...
export const ROUTES_MD = `# ShapeRouter API — Route Reference

Auth legend: **none** = public · **key** = personal API key (\`shroute_...\`) ·
...
| POST | \`/api/v1/users/:userId/api-keys\` | **fb only** | \`{ key_name }\` → the full \`shroute_...\` value, returned once |
...
export const DATA_MODEL_MD = `# ShapeRouter API — Data Model

Field names in API payloads are \`snake_case\` for ShapeRouter-owned resources
...
A personal credential (\`shroute_...\`) that authenticates its owner against the
...
interface UserApiKey {
  ...
  key_prefix: string;                          // e.g. "shroute_ab12cd" — display only
...
## Database tables (\`shaperouter\` PostgreSQL schema)
...
export const EXAMPLES_MD = `# ShapeRouter API — Worked Examples

## 0. Authenticating

Create a personal API key once, in the dashboard at
[shaperouter.com](https://shaperouter.com) → Dashboard → Settings → **Personal API
Keys** → name it → **Create key** → copy the \`shroute_...\` value (it is shown once,
though you can reveal it again from the same page).

Then use it as \`X-API-Key\` against \`https://api.shaperouter.com\`:

\`\`\`bash
curl https://api.shaperouter.com/api/v1/users/me \\
  -H "X-API-Key: shroute_..."
# {"success":true,"data":{"firebase_uid":"...","auth_method":"api_key",...}}

curl https://api.shaperouter.com/api/v1/entities \\
  -H "X-API-Key: shroute_..."
\`\`\`
...
set_credentials({ apiKey: "shroute_...", persist: true })
  -> stored in ~/.shaperouter/config.json (mode 0600); later sessions start authenticated
...
\`\`\`bash
curl -X POST https://api.shaperouter.com/api/v1/users/<uid>/api-keys \\
...
curl -X POST https://api.shaperouter.com/api/v1/ai/acme/support-tools/classify-ticket \\
  -H "Authorization: Bearer sk_live_..." \\
  -H "Content-Type: application/json" \\
  -d '{"text":"You billed me twice this month."}'

# Prompt preview - same auth, no LLM call
curl -X POST https://api.shaperouter.com/api/v1/ai/acme/support-tools/classify-ticket/prompt \\
  -H "Authorization: Bearer sk_live_..." \\
  -H "Content-Type: application/json" \\
  -d '{"text":"You billed me twice this month."}'
...
curl -G https://api.shaperouter.com/api/v1/ai/acme/support-tools/classify-ticket \\
  -H "Authorization: Bearer sk_live_..." \\
...
const res = await fetch(
  "https://api.shaperouter.com/api/v1/ai/acme/support-tools/classify-ticket",
  {
    method: "POST",
    headers: {
      Authorization: \`Bearer \${process.env.SHAPEROUTER_PROJECT_API_KEY}\`,
...
r = requests.post(
    "https://api.shaperouter.com/api/v1/ai/acme/support-tools/classify-ticket",
    headers={"Authorization": f"Bearer {os.environ['SHAPEROUTER_PROJECT_API_KEY']}"},
...
export const ERRORS_MD = `# ShapeRouter API — Errors and Troubleshooting
...
| Message | Meaning |
|---|---|
| \`No ShapeRouter credential configured...\` | Create a personal API key at shaperouter.com, then \`set_credentials({ apiKey, persist: true })\` |
| \`This operation needs a Firebase ID token...\` | Creating or revealing API keys needs a token; do it at shaperouter.com, or pass one to \`set_credentials\` |
| \`This operation needs a project API key...\` | Set \`SHAPEROUTER_PROJECT_API_KEY\`, pass \`apiKey\`, or call \`set_credentials\` |
| \`entitySlug is required...\` | Pass \`entitySlug\` or set \`SHAPEROUTER_ENTITY_SLUG\` — \`list_entities\` shows the options |
| \`orgPath is required...\` | Pass \`orgPath\` or set \`SHAPEROUTER_ORG_PATH\` / \`SHAPEROUTER_ENTITY_SLUG\` |

Firebase ID tokens expire roughly an hour after they are minted. A personal API
key (\`shroute_...\`) does not, which is why it is the credential to use for an MCP
session — create one at shaperouter.com under Dashboard → Settings → Personal API
...
it in \`~/.shaperouter/config.json\` (mode 0600) for future sessions.
...
export const PROVIDERS_MD = `# ShapeRouter API — Providers and Models
