 ...
 *
 * Location: `~/.shaperouter/config.json`, overridable with `SHAPEROUTER_CONFIG_PATH`.
...
export interface StoredConfig {
  ...
  entityApiKey?: string;
  /** Personal API key (shroute_...) for admin routes */
  apiKey?: string;
...
const CONFIG_DIRNAME = ".shaperouter";

/** Absolute path of the config file this process reads and writes. */
export function configFilePath(): string {
  const override = process.env["SHAPEROUTER_CONFIG_PATH"];
...
export function readConfigFile(): StoredConfig {
  ...
  try {
    ...
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      console.error(`[shaperouter-api] Ignoring ${path}: expected a JSON object.`);
      return {};
    }
    return parsed as StoredConfig;
  } catch (error) {
    console.error(
      `[shaperouter-api] Ignoring ${path}: ${
