 ...
 * argument, an environment variable, then `~/.shaperouter/config.json`. These
...
export function registerConfigTools(server: McpServer) {
  server.tool(
    ...
    "Show the MCP server's effective configuration: API URL, default entity slug and organization " +
      ...
      "(~/.shaperouter/config.json) holds. `adminToolsReady` covers most tools; `canManageApiKeys` is " +
  ...
  server.tool(
    ...
    "Set credentials or defaults for this session. Use it when the user pastes a personal API key " +
      "(shroute_...), a Firebase ID token, or a project API key (sk_live_...), or wants to point at a " +
      "different deployment. Only the fields you pass are changed.\n\n" +
      "Pass `persist: true` to also write them to ~/.shaperouter/config.json (mode 0600) so future " +
      ...
      "server exits. Firebase ID tokens expire in about an hour and are never worth persisting.",
    {
      entityApiKey: z
        ...
        .optional()
        .describe(
          "Entity API key (shrouteent_...) — acts as the entity itself; cannot reach /users routes " +
            "or manage API keys"
        ),
      apiKey: z
        .string()
        .optional()
        .describe("Personal API key (shroute_...) — acts as a user; does not expire"),
      authToken: z
        .string()
        .optional()
      ...
      apiUrl: z
        .string()
        .optional()
        .describe("Base URL, e.g. https://api.shaperouter.com or http://localhost:3000"),
      ...
      persist: z
        ...
        .optional()
        .describe(
          "Write the values to ~/.shaperouter/config.json for future sessions. The Firebase token is " +
  ...
  server.tool(
    ...
    "Remove the saved API key and project key from ~/.shaperouter/config.json, keeping non-secret " +
