 ...
 *
 * These are the `shroute_...` keys that authenticate their owner against the
...
export function registerApiKeyTools(server: McpServer) {
  ...
  server.tool(
    "create_api_key",
    "Create a personal API key (POST /api/v1/users/:userId/api-keys). The full `shroute_...` value is " +
      ...
      "https://shaperouter.com instead (Dashboard -> Settings -> Personal API Keys).",
    {
      userId: userIdArg,
      key_name: z
        .string()
