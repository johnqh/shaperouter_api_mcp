...
const common = {
  orgPath: z
    ...
    .optional()
    .describe(
      "Organization path — the entity slug that owns the project. Defaults to SHAPEROUTER_ORG_PATH or SHAPEROUTER_ENTITY_SLUG."
    ),
  ...
  apiKey: z
    .string()
    .optional()
    .describe("Project API key (sk_live_...) for this call; defaults to SHAPEROUTER_PROJECT_API_KEY"),
  testMode: z
    .boolean()
    .optional()
...
export function registerAiTools(server: McpServer) {
  server.tool(
    "invoke_endpoint",
    "Execute a ShapeRouter AI endpoint and get structured JSON back " +
  ...
  server.tool(
    ...
    "Return the prompt ShapeRouter would build for an endpoint WITHOUT calling the LLM " +
