...
export function registerHealthTools(server: McpServer) {
  server.tool(
    "check_api_health",
    "Check that the ShapeRouter API is reachable. `readiness: false` (default) hits GET /health " +
  ...
  server.tool(
    ...
    "Get the API banner from GET / — returns the service name, version, and status. " +
      "Useful for confirming which deployment SHAPEROUTER_API_URL points at.",
    {},
