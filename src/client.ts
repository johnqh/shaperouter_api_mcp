/**
 * @fileoverview HTTP client for the ShapeRouter API.
 ...
 *      1. An **entity API key** (`shrouteent_...`) sent as `X-API-Key`. Acts as the
 *         entity itself, so it keeps working when the member who created it
 *         leaves. Cannot reach `/users/*` or manage API keys.
 *      2. A **personal API key** (`shroute_...`) sent as `X-API-Key`. Acts as a
...
export interface ClientConfig {
  /** Base URL of the API, e.g. https://api.shaperouter.com or http://localhost:3000 */
  apiUrl: string;
  /** Entity API key (shrouteent_...) — authenticates as the entity itself */
  entityApiKey?: string | undefined;
  /** Personal API key (shroute_...) for admin routes — preferred, never expires */
...
/**
 * Resolve the entity slug for a call, falling back to SHAPEROUTER_ENTITY_SLUG.
...
export function resolveEntitySlug(slug?: string): string {
  ...
  if (!resolved) {
    throw new Error(
      "entitySlug is required. Pass it explicitly or set SHAPEROUTER_ENTITY_SLUG. " +
...
export function resolveOrgPath(orgPath?: string): string {
  ...
  if (!resolved) {
    throw new Error(
      "orgPath is required. Pass it explicitly or set SHAPEROUTER_ORG_PATH / SHAPEROUTER_ENTITY_SLUG. " +
...
function buildHeaders(auth: AuthMode, apiKeyOverride?: string): Record<string, string> {
  ...
  if (auth === "admin") {
    ...
    if (cfg.entityApiKey) {
      ...
      throw new Error(
        "No ShapeRouter credential configured. Create an entity API key at https://shaperouter.com " +
          "(Dashboard -> API Keys) or a personal API key (Dashboard -> Settings -> Personal API " +
          "Keys), then run set_credentials with it. A Firebase ID token in SHAPEROUTER_AUTH_TOKEN " +
          "also works."
      );
    }
  } else if (auth === "user") {
    ...
    if (cfg.apiKey) {
      ...
      throw new Error(
        "This route needs a personal API key (shroute_...) or a Firebase ID token. An entity API " +
    ...
    if (!cfg.authToken) {
      throw new Error(
        "This operation needs a Firebase ID token, because the API refuses to create or reveal API " +
          "keys for a caller authenticated by API key. Sign in at https://shaperouter.com and manage " +
          "keys there, or set SHAPEROUTER_AUTH_TOKEN / call set_credentials with a token."
      );
    }
    headers["Authorization"] = `Bearer ${cfg.authToken}`;
    ...
    if (!key) {
      throw new Error(
        "This operation needs a project API key (sk_live_...). Pass `apiKey`, or set SHAPEROUTER_PROJECT_API_KEY. " +
