/**
 * @fileoverview Reusable prompt templates for common ShapeRouter workflows.
 */
...
export function registerPrompts(server: McpServer) {
  server.prompt(
    ...
    "Walk through creating a working ShapeRouter endpoint for a described task, from provider key to verified invocation.",
    {
      task: z
        .string()
    ...
    ({ task, entitySlug }) =>
      userMessage(
        [
          `Set up a ShapeRouter endpoint for this task: ${task}`,
          entitySlug ? `Use the entity \`${entitySlug}\`.` : "",
          "",
          "Work through these steps, checking the result of each before moving on:",
          "1. Read `describe_shaperouter_api` (overview + examples) if you have not already.",
  ...
  server.prompt(
    "debug_endpoint",
    "Diagnose a failing or low-quality ShapeRouter endpoint.",
    {
    ...
    ({ projectName, endpointName, symptom }) =>
      userMessage(
        [
          `The ShapeRouter endpoint \`${projectName}/${endpointName}\` is misbehaving: ${symptom}`,
          "",
          "Diagnose it systematically:",
          "1. Read `describe_shaperouter_api` section 'errors' and match the symptom to a known cause.",
  ...
  server.prompt(
    "audit_entity",
    "Inventory an entity's ShapeRouter configuration and flag risks.",
    { entitySlug: z.string().optional().describe("Entity slug to audit") },
    ({ entitySlug }) =>
      userMessage(
        [
          entitySlug
            ? `Audit the ShapeRouter entity \`${entitySlug}\`.`
            : "Audit my ShapeRouter entity (use list_entities to pick it).",
          "",
